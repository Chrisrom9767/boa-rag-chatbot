# -*- coding: utf-8 -*-
"""
RAG Conformité BOA - Application Streamlit
Chatbot de conformité avec recherche vectorielle FAISS + Groq
"""
...
# ======================================================================
st.set_page_config(
    page_title="Lexi - Assistant Conformité BOA",
    page_icon="🏦",
    layout="wide",
...
def init_groq_client():
    ...
    if not api_key:
        st.error("❌ GROQ_API_KEY non définie. Veuillez configurer votre clé API dans les secrets Streamlit.")
        st.stop()
...
def load_faiss_index(_embedding_model):
    ...
    if not os.path.exists(INDEX_PATH):
        st.warning(f"⚠️ Index FAISS introuvable à {INDEX_PATH}")
        return None
    ...
    try:
        ...
        st.sidebar.success(f"✅ Index chargé: {vectorstore.index.ntotal} vecteurs")
        ...
        st.error(f"❌ Erreur de chargement FAISS: {e}")
        return None
...
def detect_lang_simple(text: str) -> str:
    """Détection simple de langue FR/EN"""
    if not text:
        return "fr"
    t = text.lower()
...
def ask_groq(client, system_prompt: str, user_question: str, context_text: str, lang: str, used_context: bool) -> str:
...
def add_footer(answer: str, sources: List[str], lang: str, used_context: bool) -> str:
    """Ajoute le footer avec sources et liens"""
    if used_context:
        if sources:
            answer += ("\n\n**Sources :**\n" if lang == "fr" else "\n\n**Sources:**\n") + "\n".join(f"- {s}" for s in sources)
            if DRIVE_LINK and "TON_LIEN" not in DRIVE_LINK:
                answer += (
                    f"\n\n📂 **Dossier source :**\n➡️ [{DRIVE_LINK}]({DRIVE_LINK})"
                    if lang == "fr" else
                    f"\n\n📂 **Source folder:**\n➡️ [{DRIVE_LINK}]({DRIVE_LINK})"
                )
    else:
...
def main():
    ...
    with st.sidebar:
        ...
        st.info(
            "**Lexi** est votre assistant IA pour les questions de conformité "
            "bancaire. Il s'appuie sur la documentation de BANK OF AFRICA."
        )
    ...
    # Titre principal
    st.title("💬 Assistant Conformité - Lexi")
    st.markdown("*Posez vos questions sur la conformité bancaire en français ou en anglais*")
    st.markdown("---")

    # Initialisation
    client = init_groq_client()
    embedding_model = load_embeddings()
    retriever = load_faiss_index(embedding_model)
            ...
            with st.spinner("🤔 Recherche en cours..."):
                ...
                if retriever is not None:
                    try:
                        ...
                        retrieval_info = f"*🔎 {len(docs)} document(s) trouvé(s) en {latency:.2f}s*"
                    except Exception as e:
                        st.warning(f"⚠️ Erreur de recherche: {e}")
                else:
                    retrieval_info = "*🔎 Mode sans base documentaire*"

                # Génération réponse
                try:
                    t0 = time.time()
                    answer = ask_groq(client, system_prompt, prompt, context_text, lang, used_context)
                    answer = add_footer(answer, sources, lang, used_context)
                    gen_time = time.time() - t0
                    
                    # Affichage
                    st.markdown(answer)
                    st.caption(f"{retrieval_info} • ⏱️ Généré en {gen_time:.2f}s")
